...
def temporal_softmax_loss(x, y, ignore_index=None):
    ...
    ##########################################################################
    # Replace "pass" statement with your code
    loss = F.cross_entropy(x.reshape(-1, x.shape[-1]), y.reshape(-1), ignore_index=-100 if ignore_index is None else ignore_index, reduction="sum") / x.shape[0]
    ##########################################################################
    #                             END OF YOUR CODE                           #
    ##########################################################################
...
class CaptioningRNN(nn.Module):
    ...
    def forward(self, images, captions):
        ...
        if self.cell_type == "attn":
            ...
            A = A.permute(0, 3, 1, 2)
            h = self.rnn(word_vectors, A)
        else:
            pooled_features = features.mean(dim=(2, 3))
            h0 = self.feature_projection(pooled_features)
    ...
    def sample(self, images, max_length=15):
        ...
        for t in range(max_length):
            ...
            if self.cell_type == "rnn":
                next_h = self.rnn.step_forward(word_vec, prev_h)
            elif self.cell_type == "lstm":
                next_h, prev_c = self.rnn.step_forward(word_vec, prev_h, prev_c)
            else:
                attn, attn_weights = dot_product_attention(prev_h, A)
                attn_weights_all[:, t] = attn_weights
                next_h, prev_c = self.rnn.step_forward(
                    word_vec, prev_h, prev_c, attn
                )
...
def dot_product_attention(prev_h, A):
    ...
    scores = torch.bmm(prev_h.view(N, 1, H), A_flat).squeeze(1)
    scores = scores / math.sqrt(H)
    weights_flat = F.softmax(scores, dim=1)
...
class AttentionLSTM(nn.Module):
    ...
    def step_forward(
        ...
        next_h, next_c = None, None
        # Replace "pass" statement with your code
        activation = x.mm(self.Wx) + prev_h.mm(self.Wh) + attn.mm(self.Wattn) + self.b
        ai, af, ao, ag = activation.chunk(4, dim=1)
        i = torch.sigmoid(ai)
        f = torch.sigmoid(af)
        o = torch.sigmoid(ao)
