...
def scaled_dot_product_two_loop_batch(
    ...
    for n in range(N):
        for i in range(K):
            scores = key[n].mv(query[n, i]) / (M ** 0.5)
            weights = F.softmax(scores, dim=0)
            out[n, i] = weights.matmul(value[n])
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return out
...
class MultiHeadAttention(nn.Module):
    ...
    def forward(
        ...
        y = self.linear(torch.cat(head_outputs, dim=-1))
        ##########################################################################
        #               END OF YOUR CODE                                         #
        ##########################################################################
        return y
...
def get_subsequent_mask(seq):
    ...
    mask = base_mask.unsqueeze(0).expand(N, -1, -1).clone()
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return mask
...
def position_encoding_simple(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    positions = torch.arange(K, dtype=torch.float32).view(1, K, 1) / K
    y = positions.repeat(1, 1, M)
    ##############################################################################
    #               END OF YOUR CODE                                             #
    ##############################################################################
    return y
...
def position_encoding_sinusoid(K: int, M: int) -> Tensor:
    ...
    ##############################################################################
    # Replace "pass" statement with your code
    positions = torch.arange(K, dtype=torch.float32).view(K, 1)
    dims = torch.arange(M)
    freq_idx = dims // 2
    exponents = ((2 * freq_idx) // M).float()
    angles = positions / torch.pow(10000.0, exponents).view(1, M)
